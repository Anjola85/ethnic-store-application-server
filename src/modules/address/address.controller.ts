...
export class AddressController {
  ...
  @Patch('update-unit')
  async updateUnit(
    @Body() addressDtoRequest: UpdateAddressDto,
    @Res() res: Response,
  ) {
    try {
      ...
      const addressRespDto: AddressRespDto =
        await this.addressService.updateUnit(addressDtoRequest, user);
      return handleCustomResponse(res, createResponse(null, addressRespDto));
    } catch (error) {
      this.logger.error(
        'Error occured in updateUnit in address.controller.ts with error: ' +
          error,
      );
