...
export class SendgridService {
  ...
  async sendWaitlistEmail(receiverEmail: string, name: string): Promise<any> {
    // email address of sender is quiikmartdev
    const senderEmailAddress = 'info@quiikmart.com';
    ...
    const socialMediaLinks = {
      instagram: 'https://www.instagram.com/quiikmart/',
      facebook: '#',
      twitter: 'https://twitter.com/quiikmart',
    };

    // Create an attachment object for each image
    const attachments = [
      {
        content:
    ...
    try {
      ...
      // throw new Error('Failed to send waitlist to email');
    }
  }
